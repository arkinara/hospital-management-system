...
type ButtonOwnProps = {
  ...
  size?: ButtonSize;
  /** Leading icon element, already rendered by the caller's injected renderer. */
...
type ButtonWithChildren = ButtonOwnProps & { children: React.ReactNode };
/**
 * Icon-only buttons MUST pass `label`; the union makes omitting it a
 * type error rather than an unlabelled control in production.
 */
type ButtonIconOnly = ButtonOwnProps & { children?: undefined; label: string };
...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(function Button(
  ...
  const {
    ...
    children,
    ...rest
  } = props as ButtonOwnProps & { children?: React.ReactNode };
