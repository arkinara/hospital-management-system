...
export const PermissionMatrix: React.FC<PermissionMatrixProps> = ({
  ...
  const toggle = (role: Role, module: ModuleName, action: PermissionAction) => {
    ...
    if (!turningOn && action === "view" && next.length > 0) {
      next = "";
      onDependencyApplied?.(
        `Create, edit and delete removed too — they require view access to ${module}.`,
      );
    }
    ...
    onChange({
      ...
      [role]: {
        ...draft[role],
        [module]: ["v", "c", "e", "d"].filter((f) => next.includes(f)).join(""),
      },
    });
  };
    ...
    <div className={cn("overflow-x-auto", className)}>
      <table className="dt">
        <caption className="sr-only">
          Permission matrix: for each module, which of view, create, edit and delete each role may
          perform
        </caption>
        <thead>
          <tr>
            ...
            {roles.map((r) => (
              <th
                ...
                className="border-l border-outline text-center"
              >
                <span className="inline-flex items-center gap-1.5">
                  {r}
                  {immutableRoles.includes(r) ? renderIcon("lock", "h-3 w-3") : null}
                </span>
              </th>
            ))}
          </tr>
          <tr>
            {roles.flatMap((r) =>
              ACTIONS.map((a, i) => (
                ...
                >
                  <span aria-hidden>{renderIcon(a.icon, "mx-auto h-3.5 w-3.5 text-muted")}</span>
        ...
        <tbody>
          {modules.map((m) => (
            <tr key={m}>
              <th
                scope="row"
                className="!static !bg-transparent text-left !text-sm font-medium !normal-case !tracking-normal"
              >
                <span className="flex items-center gap-2">
                  {renderIcon(MODULE_ICON[m] ?? "box", "h-4 w-4 text-muted")}
                  {m}
                </span>
              </th>
              {roles.flatMap((r) =>
                  ...
                  return (
                    ...
                    >
                      <input
                        ...
                        className="h-4 w-4 cursor-pointer align-middle accent-[rgb(var(--primary))] disabled:cursor-not-allowed disabled:opacity-40"
                      />
  ...
  (Object.keys(draft) as Role[]).forEach((role) =>
    modules.forEach((module) =>
      (["view", "create", "edit", "delete"] as PermissionAction[]).forEach((action) => {
        ...
        if (now !== can(saved[role][module] ?? "", action))
          out.push({ role, module, action, granted: now });
      }),
    ),
  );
  return out;
