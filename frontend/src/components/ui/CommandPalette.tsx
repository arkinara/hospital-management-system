  ...
  return (
    <>
      <div
        ...
        onClick={onClose}
      />
      <div className="pointer-events-none fixed inset-0 z-overlay grid place-items-start overflow-y-auto p-3 sm:place-items-center sm:p-4">
        <div
          role="dialog"
          aria-modal="true"
          aria-label="Search or jump to"
          className="anim-sheet pointer-events-auto w-full rounded-2xl border border-outline bg-surface-0 p-4 shadow-overlay sm:max-w-2xl"
        >
          <div className="relative">
            ...
            <input
              ...
              onKeyDown={onKeyDown}
              className="focus-inset min-h-12 w-full rounded-lg border border-outline-strong bg-surface-2 pl-9 pr-3 text-md"
            />
          </div>
          ...
          <ul
            ...
            role="listbox"
            aria-label="Results"
            className="-mx-1 mt-3 max-h-80 overflow-y-auto px-1"
          >
            {results.length === 0 ? (
                ...
                return (
                  <React.Fragment key={item.id}>
                    {header ? (
                      <li
                        role="presentation"
                        className="px-2 pb-1 pt-3 text-2xs font-semibold uppercase tracking-wide text-subtle"
                      >
                        {header}
                      </li>
                    ) : null}
                    <li
                      ...
                      onClick={() => run(index)}
                      className={cn(
                        "flex min-h-11 cursor-pointer items-center gap-3 rounded-lg px-2.5 py-2",
                        index === active
                          ? "bg-primary-container text-primary-container-foreground"
                          : "hover:bg-surface-2",
                      )}
                    >
                      <span className="shrink-0 text-muted">{renderIcon(item.icon, "h-4 w-4")}</span>
                      <span className="min-w-0 flex-1">
                        <span className="block truncate text-base font-medium">{item.label}</span>
                        <span className="num block truncate text-xs text-muted">{item.meta}</span>
                      </span>
                      {renderIcon("arrow-right", "h-3.5 w-3.5 text-subtle")}
                    </li>
                  </React.Fragment>
                );
              })
          ...
          <p className="mt-3 flex flex-wrap gap-x-4 gap-y-1 border-t border-outline pt-3 text-xs text-subtle">
            <span>
